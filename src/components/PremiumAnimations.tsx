...
export default function PremiumParticles() {
  ...
  useEffect(() => {
    ...
    const createParticle = (): Particle => ({
      ...
      color: ['#22c55e', '#16a34a', '#4ade80', '#d4a574', '#06b6d4'][
...
export function GlowingBorder({ children, color = 'green' }: { children: React.ReactNode; color?: string }) {
  ...
  return (
    ...
    >
      <div className={`absolute -inset-0.5 bg-gradient-to-r ${colorMap[color] || colorMap.green} rounded-2xl blur opacity-30 group-hover:opacity-60 transition-opacity duration-300`} />
...
export function PulseRing({ size = 60, color = 'green' }: { size?: number; color?: string }) {
  const colorMap: Record<string, string> = {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
          className={`absolute inset-0 rounded-full border-2 ${colorMap[color] || colorMap.green} opacity-0`}
          animate={{
            scale: [1, 2],
            opacity: [0.6, 0],
          }}
